...
#define PROTOCOL_VERSION 1    //!< The version of the protocol
// MAX_PAYLOAD = MAX_PAYLOAD_SIZE from RF24Netwrok lib = 144;
...
typedef struct {
	...
	sensor_type sensorCommand; // 1 byte
	sensor_type sensorType; // 1 byte
	system_message_type messageType; // 1 byte
	payload_type datatype; // 1 byte
  char payload[139]; //char* payload; // (MAX_PAYLOAD_SIZE - other fields; //144 - (1 + 1 + 1 + 1 + 1 ) = 139 byte before fragmentation;
} SendingMessage; // size = 144;
...
{
public:
  SendingMessage internalMessage;
	// Constructors
	Message();
  SendingMessage* getInternalMessage() {return &internalMessage;}
  ...
  Type getPayload() const {
    return *static_cast<Type const * const>
    (
        static_cast<void const * const>(internalMessage.payload)
    );
  }
	...
	Message& setSensor(uint8_t sensor);
	Message& setDestination(uint8_t destination);

	// Setters for payload
	Message& set(void* payload, uint8_t length);
	Message& set(const char* value);
	Message& set(float value, uint8_t decimals);
	Message& set(bool value);
	Message& set(uint8_t value);
	Message& set(uint32_t value);
	Message& set(int32_t value);
	Message& set(uint16_t value);
	Message& set(int16_t value);
